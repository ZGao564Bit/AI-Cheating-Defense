...
# 2. output the result to the file
os.system("mkdir output")


def write_out_file(outfilename, frame):
    '''
    outfilename: output the name of file
    frame: image needs to be saved
    '''
    cv.imwrite(outfilename, frame)


# 3. adjust the frames of video
total_frame = 378
frame_per_second = 30  # setup the frame per second
...
#cap = cv.VideoCapture(ImageGrab.grab())
num = cap.get(cv.CAP_PROP_FRAME_COUNT)
step = int(num/total_frame)
filename_base = args.input.split(".")[0]
index = 0
while cv.waitKey(1) < 0:
    ...
    # if auto:
        ...
        # pyautogui.moveTo(points[0][0], points[0][1], duration=dur)
    if auto:
    ...
    cv.imshow('OpenPose Pro', frame)
    write_out_file(".\\output\\"+filename_base +
                   "_"+str(index)+".jpg", frame)
